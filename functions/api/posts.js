export async function onRequest(context) {
  try {
    ...
    // ★ここでDBバインドが取れているか確認（取れてなければ原因が一発でわかる）
    if (!env || !env.DB) {
      return json({ ok: false, error: "env.DB is missing (D1 binding name mismatch?)" }, 500);
    }

    // ★DBに接続できるか超簡易テスト
    // これが失敗すると、D1のバインド/権限/環境が原因
    ...
    if (request.method === "POST") {
      ...
      // Turnstile 検証（※まだキー未設定なら、POSTは後で。GETの疎通だけ先に見ます）
